...
export class AudioSystem {
  ...
  /** Paměť přehrávání před timeline hold (obnoví jen to, co opravdu hrálo). */
  private playbackHoldRestore = new Map<HTMLAudioElement, boolean>();
  ...
  async preload(
    ...
    for (let i = 0; i < all.length; i++) {
      ...
      await this.preloadOne(kind, file);
  ...
  stopAll(): void {
    this.playbackHoldRestore.clear();
  ...
  /** Pozastaví všechny stopy bez resetu pozice (timeline alert). */
  pausePlayback(): void {
    this.playbackHoldRestore.clear();
    const seen = new Set<HTMLAudioElement>();
    const capture = (a: HTMLAudioElement) => {
      if (seen.has(a)) return;
      seen.add(a);
      const wasPlaying = !a.paused && !a.ended;
      this.playbackHoldRestore.set(a, wasPlaying);
      a.pause();
    };
    for (const a of this.pool.values()) capture(a);
    for (const a of this.byId.values()) capture(a);
  }

  resumePlayback(): void {
    for (const [a, was] of this.playbackHoldRestore) {
      if (was) void a.play().catch(() => {});
    }
    this.playbackHoldRestore.clear();
  }
